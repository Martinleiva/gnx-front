...
const NUEVO_TITULO = gql`
    mutation addtitle($input: TitleTypeInput!) {
...
const NuevoTitulo = () => {
    ...
    const [ addtitle ] = useMutation(NUEVO_TITULO, {
        update(cache, { data: { addtitle }}) {
            //Obtener el objeto de cache que deaseamos actualizar
            const { titles } = cache.readQuery({ query:  LISTADO_TITULOS });

            //Reescribimos el cache (cache nunca se debe modificar direactamente)
            cache.writeQuery({
                query: LISTADO_TITULOS,
                data: {
                    titles: [...titles, addtitle]
                }
            });
        }
    });
    ...
    const formik = useFormik({
        ...
        onSubmit: async valores => {
            ...
            const { title, from_date, to_date } = valores

            try {
                // eslint-disable-next-line
                const { data } = await addtitle({
                    variables: {
                        input: {
                            title,
                            from_date,
                    ...
                    >
                        ...
                        <div className="mb-4">
                            ...
                            <input 
                                ...
                                id="title"
                                type="text"
                                placeholder="Título del Empleado"
